...
import { Clock, Search, Download, AlertTriangle, Users, ChevronRight } from "lucide-react";

const RETIREMENT_AGE = 58;

function getRetirementInfo(emp: Employee) {
  ...
  return { retirementDate, remainDays };
}
...
export default function RetirementPage() {
  ...
  const retiringEmployees = employees
    .filter(emp => {
      if (emp.status !== "aktif") return false;
      const info = getRetirementInfo(emp);
      if (!info) return false;
      return info.remainDays > 0 && info.remainDays <= maxDays;
    })
    .map(emp => ({ ...emp, ...getRetirementInfo(emp)! }))
    .sort((a, b) => a.remainDays - b.remainDays);

  const filtered = retiringEmployees.filter(emp =>
    emp.fullName.toLowerCase().includes(search.toLowerCase()) ||
    emp.nip.toLowerCase().includes(search.toLowerCase())
  );

  const urgentCount = retiringEmployees.filter(e => e.remainDays <= 90).length;
  const soonCount = retiringEmployees.filter(e => e.remainDays <= 365).length;
  ...
  const exportToExcel = async () => {
    ...
    const data = filtered.map((emp, i) => ({
      ...
      "Tanggal Pensiun": emp.retirementDate.toLocaleDateString('id-ID'),
      "Sisa Hari": emp.remainDays,
    }));
    const ws = XLSX.utils.json_to_sheet(data);
    ws["!cols"] = [{ wch: 5 }, { wch: 30 }, { wch: 16 }, { wch: 22 }, { wch: 16 }, { wch: 16 }, { wch: 10 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Daftar Pensiun");
    XLSX.writeFile(wb, `Daftar_Pensiun_${periodFilter}Tahun.xlsx`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight" data-testid="text-page-title">Daftar Akan Pensiun</h1>
          <p className="text-sm text-muted-foreground mt-1">Pegawai yang mendekati masa pensiun (usia {RETIREMENT_AGE} tahun)</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-muted-foreground font-medium">Total Akan Pensiun</p>
              <p className="text-xl font-bold" data-testid="text-total-retiring">{retiringEmployees.length}</p>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-muted-foreground font-medium">Pensiun &lt; 1 Tahun</p>
              <p className="text-xl font-bold" data-testid="text-soon-retiring">{soonCount}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-muted-foreground font-medium">Pensiun &lt; 3 Bulan</p>
              <p className="text-xl font-bold" data-testid="text-urgent-retiring">{urgentCount}</p>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="text-left px-5 py-3 font-medium text-muted-foreground hidden lg:table-cell">Tgl Lahir</th>
                  <th className="text-left px-5 py-3 font-medium text-muted-foreground">Tgl Pensiun</th>
                  ...
                  return (
                    <tr key={emp.id} className="hover:bg-muted/40 transition-colors" data-testid={`retirement-row-${emp.id}`}>
                      ...
                      <td className="px-5 py-3.5 text-muted-foreground hidden lg:table-cell">
                        {emp.birthDate ? new Date(emp.birthDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : "—"}
                      </td>
                      <td className="px-5 py-3.5">
                        {emp.retirementDate.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </td>
                      <td className="px-5 py-3.5">
                        <Badge
            ...
            {filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-sm font-medium">Tidak ada pegawai yang akan pensiun</p>
